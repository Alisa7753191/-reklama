...
const ADMINISTRATOR_NAME = 'Алиса Новикова'
const ADMINISTRATOR_INITIALS = 'АН'
...
function ReviewPage({ review, client, settings, team, onBack, onUpdate }: { review: ReviewMatter; client: Client | undefined; settings: WorkspaceSettings; team: TeamMember[]; onBack: () => void; onUpdate: (review: ReviewMatter) => void }) {
  ...
  const approvalBlocked = Boolean(review.report?.findings.some((finding) => (
    finding.id === 'obscene_or_offensive_language' || finding.id === 'violence_or_cruelty_language'
  )))
  ...
  function changeStatus(status: ReviewStatus) {
    if (status === 'approved' && approvalBlocked) {
      setError('Согласование заблокировано: удалите запрещённое выражение и повторно проверьте материал.')
      document.getElementById('ad-text-editor')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
      return
    }
  ...
  return <>
    ...
    <div className="review-workbench"><main>
      ...
      <section className="workspace-card review-control"><span>РЕШЕНИЕ ЮРИСТА</span><h2>Статус и ответственный</h2><label>Статус<select value={review.status} onChange={(event) => changeStatus(event.target.value as ReviewStatus)}>{STATUS_ORDER.map((status) => <option key={status} value={status} disabled={status === 'approved' && approvalBlocked}>{STATUS_LABEL[status]}</option>)}</select></label><label>Юрист<select value={review.reviewer ?? ''} onChange={(event) => onUpdate({ ...review, reviewer: event.target.value || null, updatedAt: new Date().toISOString() })}><option value="">Не назначен</option>{team.filter((member) => member.active).map((member) => <option key={member.id} value={member.name}>{member.name}</option>)}</select></label>{review.reviewer && <p className="reviewer-line">✓ {review.reviewer}</p>}</section>
